...
public enum Command {
    ...
    put("Upload a file or folder, into a zip archive if the remote path is in one.", "put <--skip-existing> local-path <remote-path> ", Set.of(Flag.SKIP_EXISTING), Argument.LOCAL_FILE, Argument.REMOTE_FILE),
    ...
    rm("Remove a remote file, or an entry in a zip archive.", "rm remote-path", Argument.REMOTE_FILE),
