...
public class PeergosNetworkUtils {
    ...
    public static void uploadDirToFriend(NetworkAccess network, Random random) {
        ...
        List<FileWrapper.FileUploadProperties> files = List.of(
                new FileWrapper.FileUploadProperties("asmallfile1.txt",
                        () -> AsyncReader.build(smallFileData), 0, smallFileData.length, Optional.empty(), Optional.empty(), false, false, x -> {}),
        new FileWrapper.FileUploadProperties("afile.txt",
                        () -> AsyncReader.build(fileData), 0, fileData.length, Optional.empty(), Optional.empty(), false, false, x -> {}),
                new FileWrapper.FileUploadProperties("asmallfile2.txt",
                        () -> AsyncReader.build(smallFileData), 0, smallFileData.length, Optional.empty(), Optional.empty(), false, false, x -> {})
        );
        String subdirName = "subdir";
        Stream<FileWrapper.FolderUploadProperties> upload = Stream.of(
