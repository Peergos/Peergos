...
//            Logging.LOG().info("Req host: " + reqHost + ", isSub: " + isSubdomain + ", path: " + path);
            ...
            if (includeCsp)
                httpExchange.getResponseHeaders().set("content-security-policy", "default-src 'self' " + this.host + ";" +
                        ...
                        "frame-src 'self' " + frameDomains.stream().collect(Collectors.joining(" "))
                        + " " + blockstoreDomain.stream().map(d -> " https://" + d + "/fetch-robot-proxy.html").collect(Collectors.joining())
                        + " " + (isSubdomain ? "" : this.host.wildcard()) + ";" +
