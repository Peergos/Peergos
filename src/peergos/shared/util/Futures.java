...
public class Futures {
    ...
    public static <T> CompletableFuture<Set<T>> combineAll(Collection<CompletableFuture<T>> futures) {
        ...
        List<Throwable> inError = new ArrayList();
        return futures.stream().reduce(identity,
                (a, b) -> b.thenCompose(opt -> {
                        if(! inError.isEmpty()) {
                            throw new RuntimeException(inError.get(0));
                        }
                        return a.thenApply(set -> Stream.concat(set.stream(), Stream.of(opt))
    ...
    public static <T> CompletableFuture<List<T>> combineAllInOrder(Collection<CompletableFuture<T>> futures) {
        ...
        return futures.stream().reduce(identity,
                (a, b) -> b.thenCompose(opt -> {
                        ...
                        return a.thenApply(set -> {
                            List<T> combined = new ArrayList<>(set.size() + 1);
                            combined.addAll(set);
                            combined.add(opt);
                            return combined;
                        }).exceptionally(t -> {
