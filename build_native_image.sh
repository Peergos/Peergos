...
$NATIVE_IMAGE_BIN -H:EnableURLProtocols=http -H:+ReportUnsupportedElementsAtRuntime -H:ConfigurationFileDirectories=META-INF/native-image --no-server --no-fallback --initialize-at-build-time=org.sqlite.DB,org.sqlite.NativeDB -jar "$PEERGOS_JAR_PATH" peergos
